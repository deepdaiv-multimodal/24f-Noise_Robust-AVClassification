...
def train(audio_model, train_loader, test_loader, args):
    ...
    while epoch < args.n_epochs + 1:
        ...
        stats_audio_only, valid_loss_audio_only = validate(audio_model, test_loader, args, noise_to_audio=False, noise_to_vision=True)
        stats_vision_only, valid_loss_vision_only = validate(audio_model, test_loader, args, noise_to_audio=True, noise_to_vision=False)
        stats_both, valid_loss_both = validate(audio_model, test_loader, args, noise_to_audio=True, noise_to_vision=True)
        ...
        stats_list = [stats_complete, stats_audio_only, stats_vision_only, stats_both]
        valid_loss_list = [valid_loss_complete, valid_loss_audio_only, valid_loss_vision_only, valid_loss_both]
        ...
        with open(exp_dir + '/stats_' + str(epoch) +'.pickle', 'wb') as handle:
            pickle.dump(stats_complete, handle, protocol=pickle.HIGHEST_PROTOCOL)
        _save_progress()
...
def validate(audio_model, val_loader, args, output_pred=False, noise_to_audio=False, noise_to_vision=False):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
